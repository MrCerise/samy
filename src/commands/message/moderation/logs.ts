...
export default new MessageCommand({
  ...
  subcommands: [
    ...
    new MessageSubcommand({
      ...
      async execute(client, message) {
        await runRouteList(client, message);
      },

      subcommands: [
        new MessageSubcommand({
          name: "add",
          description: "Route a specific log type from one channel to another.",
          category: "Moderation",
          guildOnly: true,
          userPermissions: ["ManageGuild"],
          arguments: [
            channelArg(
              "source",
              "The channel, thread, or forum logs originate from.",
            ),
            channelArg(
              "target",
              "The channel, thread, or forum post to send these logs to instead.",
            ),
            categoryArg("log_type"),
          ],

          async execute(client, message, args) {
            const sourceRaw = args.getString("source");
            const targetRaw = args.getString("target");
            const logType = args.getString("log_type");

            if (!sourceRaw || !targetRaw || !message.guild) {
              await reply(
                message,
                client.i18n.t("commands.logs.channel_not_found"),
              );
            ...
            const source = await resolveChannelLike(message.guild, sourceRaw, {
              strict: false,
            });
            if (!source.ok) {
              await reply(message, channelErrorMessage(client, source.error));
              return;
            }

            const target = await resolveChannelLike(message.guild, targetRaw, {
              strict: true,
            });
            if (!target.ok) {
              await reply(message, channelErrorMessage(client, target.error));
              return;
            }
            ...
            await reply(
              message,
              client.i18n.t("commands.logs.route.add_success", {
                source: source.channel.mention,
                target: target.channel.mention,
                category: logType,
              }),
            );
          },
        ...
        new MessageSubcommand({
          name: "remove",
          description:
            "Remove a route for a single log type from a source channel.",
          category: "Moderation",
          guildOnly: true,
          userPermissions: ["ManageGuild"],
          arguments: [
            channelArg(
              "source",
              "The source channel, thread, or forum to stop routing.",
            ),
            categoryArg("log_type"),
          ],
          ...
          async execute(client, message, args) {
            ...
            await reply(
              message,
              client.i18n.t("commands.logs.route.remove_success", {
                source: source.channel.mention,
                category: logType,
              }),
            );
        ...
        new MessageSubcommand({
          name: "clear",
          description: "Clear all routes for a source channel.",
          category: "Moderation",
          guildOnly: true,
          userPermissions: ["ManageGuild"],
          arguments: [
            channelArg(
              "source",
              "The source channel, thread, or forum to clear routes for.",
            ),
          ],
          ...
          async execute(client, message, args) {
            ...
            await reply(
              message,
              client.i18n.t("commands.logs.route.clear_success", {
                source: source.channel.mention,
              }),
            );
          },
...
async function runRouteList(client: Client, message: Message) {
  if (!message.guild) {
    await reply(message, client.i18n.t("commands.lockdown.guild_only"));
    return;
  }

  const routes = await listRoutes(client, message.guild.id);

  if (routes.length === 0) {
    await reply(message, client.i18n.t("commands.logs.route.none"));
    return;
  }

  const lines = routes.map(
    (entry) =>
      `- <#${entry.sourceChannelId}> **(${entry.category.toLowerCase()})** → <#${entry.targetChannelId}>`,
  );

  await reply(
    message,
    client.i18n.t("commands.logs.route.title", {
